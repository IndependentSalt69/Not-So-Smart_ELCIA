...
class Incident(Base):
    ...
    @property
    def source(self) -> str:
        """Identifies whether this incident was generated by AI vision or human reported."""
        if "-M" in self.incident_code:
            return "HUMAN_REPORTED"
        if hasattr(self, "status_history") and self.status_history:
            for hist in self.status_history:
                if hist.comment and ("MANUAL_REVIEW" in hist.comment or "manual_override=true" in hist.comment):
                    return "HUMAN_REPORTED"
        return "AI_VISION"
